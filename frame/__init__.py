...
class LatentDrichAllocation:
    def __init__(self,filename, topics, alpha, beta, docs, vocabulary_size, filenameJSON, smart_initialize=True):
        self.topics = topics
...
def lda_generate_learn(lda, iteration, voca):
    ...
    # Perplexity is dened as the geometric mean of the inverse marginal probability of each word in the held-out set of document
...
def output_word_topic_dist(lda, voca):
    ...
    filenameJSONW = 'topicsfile_'+str(lda.filenameJSON)+'_'+str(lda.topics) + '.json' 
    fj = open(filenameJSONW,'w')
    fj.write("{\"topics\": [")
    ...
    fp = open (filenameJSONW,"r")
    lda_data_txt = json.load(fp)
    fp.close()

    fp = open (filenameJSONW,"w")
    json.dump(lda_data_txt, fp, indent=4)
    fp.close()
...
def generateTopicsParams(filename, topics,iterations, alpha_param, beta_param, filenameJSON, GRAM):
    if alpha_param == None:
        alpha_param=0.5
    if beta_param == None:
        beta_param=0.5
    ...
    corpus=corpusvector.load_file(filename, GRAM)
    vocab = corpusvector.CorpusVectors()
    docs = [vocab.doc_to_ids(doc) for doc in corpus]
    # alpha = distributions for (per document) topic & beta = (per topic) word distributions
    # smaller alpha = more sparse the distribution
    ...
    # see example here, http://www.mblondel.org/journal/2010/08/21/latent-dirichlet-allocation-in-python/
    # we can perhaps provide an interface for these parameters later
    lda_object= LatentDrichAllocation(filename, topics, alpha_param, beta_param, docs, vocab.size(),filenameJSON, True)
    x = vocab.size()
    print "vocab size = % d" % x
    lda_generate_learn(lda_object, iterations, vocab)
